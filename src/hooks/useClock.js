...
import { getTimeComponents, padTimeUnit } from '../utils/getTime';
...
export const useClock = () => {
    ...
    // Settings and alarms: integrate with settings and alarm checker hooks
    ...
    const formatTime = useCallback(
        (hours, minutes, format = settings.hourFormat) => {
            if (format === '24h') {
                return `${hours}:${padTimeUnit(minutes)}`;
            } else {
                // 12-hour format
                ...
                return `${displayHour}∶${padTimeUnit(minutes)} ${am_pm}`;
            }
        },
        [settings.hourFormat],
    );
    ...
    const getReminderInfo = useCallback(() => {
        ...
        const currentMinute = padTimeUnit(minutes);
        ...
        const getNextReminder = () => {
            ...
            const nextInHour = sortedReminders.find(
                (reminderMin) => reminderMin > minutes,
            );
        ...
        return {
            ...
            enabledTimes: activeReminders.sort(
                (a, b) => parseInt(a) - parseInt(b),
            ),
        };
    }, [settings.reminders, hours, minutes, formatTime]);

    // Animation helper for smooth transitions
    const animateIfSmallChange = useCallback(
        (motionValue, targetAngle, duration = 0.3) => {
