...
// Main provider component for app-wide settings
const SettingsProvider = ({ children }) => {
            ...
            const merged = {
                darkMode:
                    typeof savedSettings.darkMode === 'boolean'
                        ...
                        : defaults.darkMode,
                hourFormat: ['12h', '24h'].includes(savedSettings.hourFormat)
                    ? savedSettings.hourFormat
                    : defaults.hourFormat,
                reminders: Array.isArray(savedSettings.reminders)
                    ? savedSettings.reminders
                    : defaults.reminders,
                sound: {
                    volume:
                        typeof savedSettings.sound?.volume === 'number' &&
                        savedSettings.sound.volume >= 0 &&
                        savedSettings.sound.volume <= 100
                            ? savedSettings.sound.volume
                            : defaults.sound.volume,
                    selectedSound: ['chime1', 'double-chime'].includes(
                        savedSettings.sound?.selectedSound,
                    )
                        ? savedSettings.sound.selectedSound
                        : defaults.sound.selectedSound,
                    enabled:
                        typeof savedSettings.sound?.enabled === 'boolean'
                            ? savedSettings.sound.enabled
                            : defaults.sound.enabled,
                },
                remindUntil: {
                    time:
                        typeof savedSettings.remindUntil?.time === 'string' &&
                    ...
                    enabled:
                        typeof savedSettings.remindUntil?.enabled === 'boolean'
    ...
    const [settings, setSettings] = useState(() => {
        if (!isLocalStorageAvailable()) {
            Logger.warn(
                'localStorage not available, using defaults with system preference',
            );
            return { ...defaultSettings, darkMode: getSystemDarkPreference() };
        }
        ...
        if (savedSettings) {
            return validateAndMergeSettings(savedSettings, defaultSettings);
        }

        Logger.log(
            'No saved settings found, using defaults with system preference',
        );
        return { ...defaultSettings, darkMode: getSystemDarkPreference() };
    });
    ...
    useEffect(() => {
        ...
        Logger.log(
            `Setting theme to: ${theme} (darkMode: ${settings.darkMode})`,
        );
